...
def gen_training_data(k, low=[0, 0], high=[1, 1], n_b=10, n_df=10):

    X_boundary = util.boundary_X_2d(low, high, n_b)
    bc_index = np.arange(0,X_boundary.shape[0])
    bc_index_sample = np.random.choice(bc_index,n_b,replace=False)
    X_boundary = X_boundary[bc_index_sample]
